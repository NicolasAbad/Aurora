...
**Resolved (Sprint 11.5): raised Fabrication's per-unit consumption, §4's chosen lever, not Refinery's or Supply Depot's own rate.** At equal building levels L, Fabrication+Refinery's combined draw was `0.6L + 0.5L = 1.1L` against Supply Depot's `1.5L` output — comfortably positive even fully staffed, which is why Materials was never really scarce. Raising Fabrication 2→3 Materials/Hardware (§4) changes that to `0.9L + 0.5L = 1.4L` vs `1.5L` — a real, felt squeeze at matched levels, and a real deficit the instant Supply Depot lags behind. Chose Fabrication over Refinery because it's the larger, more central consumer (Hardware feeds certifications and every launch); chose NOT to touch Supply Depot's own output, since lowering supply directly would have punished Materials-independent play (sonda era, pre-Fabrication) for no reason tied to the actual finding.
...
**New data point from item 3/4's own sim-verification (before this item's own work has started):** with Engineer/Scientist now promotion-only and the Funding cap at 1,000, a 10-seed human-profile sweep shows the OPPOSITE problem at the START of the tree from the abundance finding above — `basicEngineering` (first Program-branch node, no deps) sits eligible-but-unaffordable for 12+ simulated hours in every seed, and sonda-era Flight Data share fell to a median 16.4% (target 20-35%), down from 28% with item 3 alone. Read together: Research is scarce getting the FIRST Scientist promoted (a new bootstrap-speed problem, not a units-abundance one), then presumably still abundant once 3+ Scientists are staffed later (the original finding, not re-verified since). Both ends of this curve are this item's job to address — starting-node cost/pacing and later-tree abundance may need different fixes, not one uniform income change.

**Resolved (Sprint 11.5).** Implemented: (a) `probe1Engine` gains `buildingDep: testStand` (the named first case — certifying an engine-test procedure for a facility you don't have was the real complaint) plus a 50-Materials secondary cost; `orbital1Engine` (transitively covered by probe1Engine's buildingDep, no separate check needed) gains 200 Materials. (b) Secondary costs added to 5 nodes total, reserved for certification-adjacent and building-unlocking nodes per the doc's own guidance: `probe1Engine`/`orbital1Engine`/`autoRefuel` (Materials — real hardware/plumbing), `testStand`/`flightProgram` (Funding — these unlock the Testing/Launch complexes). Left pure-Research: `aluminum`, `soundingRockets`, `basicLogistics`, `remoteOps`, `vabQueues`, `basicEngineering`, `scientificMethod`, `titanium`, `flightOperations` — no "real money" justification for these beyond the two building-unlockers already covered. (c) Income rebalance: raised ONLY `orbitalFlight` (700→1,000R, the capstone node the "3 Scientists out-produce the tree" finding most concretely names — the longest single node at 6h, and it gates Aurora II's SECOND attempt onward, not Aurora I). Deliberately did NOT touch R&D Lab's base rate or any early/mid node's Research cost, and did NOT lower income — fresh sim data (above) showed active EARLY scarcity, and either move would have worsened an already-confirmed regression instead of fixing the later-tree abundance this item was actually about.

**Real bug found and fixed during this item's own sim-verification, not a value change:** `sim/run.ts`'s `resolvePromotions` only ever promoted engineers toward R&D Lab's Scientist requirement — once that target was met, it stopped touching engineer count entirely, so Fabrication/Refinery's own engineer slots (task 3 removed their direct-hire path) silently went unfilled forever. Manifested as the "optimal" profile never producing Hardware, never building Test Stand, never reaching Aurora I at all within 30 days — informational-only, so it wasn't gating anything, but it would have quietly corrupted every sim-verification this session ran against a broken bot model. Fixed: `resolvePromotions` now holds back `requiredSlots('engineer')` (Fabrication + Refinery's own need) as a reserve before promoting any surplus to Scientist, and — the actual bug in the first attempted fix — keeps working as long as EITHER the Scientist target or the reserve is unmet, not just the Scientist target. After the fix, all three profiles reach Aurora II (optimal day 2, human day 6, casual day 13) and the sonda Flight Data share recovered to 20.2% median (back in the 20-35% band).

**New finding, NOT resolved here — flagged for item 8/9:** with the bot fixed and correctly staffing Fabrication/Refinery, the human profile's salary ratio is now 62–69.5% against the 30–55% target (was artificially reading in-band before, suppressed by the broken bot under-hiring engineers/controllers). This is a real, current sanity-rule violation, but it's a cumulative effect of tasks 3/4/6/7 together now being visible for the first time with correct staffing — not something to patch inside this item. Item 8 (hiring-cost curve) and item 9 (multi-resource upgrade costs) are both directly about hiring/staffing cost levers and are where this belongs.
