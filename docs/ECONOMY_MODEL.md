...
**v4.2b reconciliation (CLAUDE.md rule 5b):** the incoming v4.2 replacement was authored from a pre-v4.1 base (it never received the v4.1 changelog entry below) and silently reverted/dropped several things Sprint 11.5 shipped, sim-verified, and closed: **starting Funding cap** (reverted to 500, candidate-framed — shipped value is 1,000, `initialState.ts`); **Fabrication's Materials-per-Hardware** (reverted to 2 — shipped value is 3, `buildings.ts`, confirmed live via Playwright showing "Consumes: 3 per Hardware" on the tile); **the hiring-cost exponent** in §3's own formula line (reverted to 1.15 while a few lines below it correctly says "raised 1.15→1.20 and closed" — self-contradictory; shipped `HIRING_COST_EXPONENT` is 1.2); **Finance's §4d multi-resource cost thresholds** (dropped entirely — shipped `costThresholds: [{level:20, materials:40}, {level:50, hardware:15}]` still lives in `buildings.ts` and is sim-tested); and the §4d/§5b "Resolved" status writeups (both dropped, leaving shipped work phrased as an open design goal). Also fixed a dangling `§1e` reference in §5c (no such section exists here — it meant UI_SPEC §1d). All restored/fixed below; the genuine new content in this replacement — the v4.2 findings summary, new §3e retune scope, new §5c research redesign direction, and the Aluminum alloys doc-sync (correct, matches shipped code) — is kept as authored. Flagging because Sprint 11.6 sim-verifies its research redesign against this file; verifying against a doc that thinks the economy is still pre-Sprint-11.5 would corrupt that work.

**v4.2 changes (dual-audit findings, post-Sprint-11.5):** Aluminum alloys doc-synced to its real shipped effect (−10% Materials/Hardware, was documented as none). **Major finding, NOT yet acted on — needs the sim-verified retune below before Sprint 12:** salary ratio still out of band (62–69.5% vs 30–55%, unresolved across two prior attempts); Research/Flight XP/Reputation overabundant by 75×/20×/92× against total tree cost — three of eight resources stop meaning anything by day 5-6 of a 30-day-tuned arc; contract income is 0.22% of passive Finance income (economically irrelevant); `orbitalFlight`'s gate doesn't actually separate Aurora II from Aurora I in time (both land the same day, every profile) — the v1 finale currently fires simultaneous with, not after, the climax. **§3e below sets the retune's scope and boundary.**

**v4.1 changes (Sprint 11.5 — economy friction & progression pass, owner manual-play findings post-Sprint-11):** Engineer/Scientist made promotion-only (structural, not a value patch — GDD §2 v2.11); `basicEngineering`/`scientificMethod` repurposed as promotion accelerators. New §3b (resource friction principles) and §5b (research tree gating + mixed costs) added. Starting Funding cap raised 500→1,000; Fabrication's Materials-per-Hardware raised 2→3 (§3b's chosen friction lever); hiring-cost exponent raised 1.15→1.20 (§3d); Finance gained multi-resource cost thresholds past level 20/50 (§4d). Real bug flagged and fixed: displayed Funding rate must net out salary burn (§3c) — was showing gross production, not what the player actually receives.
...
## 1. Starting state
Funding 0 · Materials 0 · Research 0 · Hardware 0 (Aluminum tier) · Propellant 0 · Reputation 0 · Flight XP 0. Buildings: Offices lv1 (free, pre-built). Staff: 0. **Starting staff cap: 2** (before any Crew Quarters; Quarters adds +3 per level). Initial caps (no Warehouse): Funding **1,000** (v4.1, Sprint 11.5 — raised from 500, owner-reported feeling too tight; sim-verified: human-profile pacing floor/ceiling both still hold, Aurora I lands right at day 5 across a 10-seed sweep) · Materials 200 · Hardware 50 · Propellant 0 (requires Propellant Depot).
...
## 3. Staff — hiring cost `base × 1.20^hiredOfThatRole` (exponent per role, Technician/Controller only, raised from 1.15 in Sprint 11.5 — see §3d's status note below), salary per second per unit
...
**Status (v4.2): Sprint 11.5 raised the exponent 1.15→1.20 and closed this item — but a fresh 30-day run with the now-standard `aggressive` profile shows the SAME trivial-hire experience persists (Finance level 15, next hire = 2.9s of income, next promotion = 3.3s). The fix reduced but did not resolve the underlying finding. Re-opened — folded into §3e's retune, not treated as closed.**

## 3e. Numbers-only retune scope (post-dual-audit, before Sprint 12) — bounded on purpose
This retune is explicitly SCOPED to fixing magnitudes that are wrong regardless of taste — not to adding sinks, mechanics, or new content. That distinction matters: whether the game NEEDS new late-game systems (Program Policies spending Reputation, deeper research trees, repeatable Flight XP tiers — all already in BACKLOG) is a real question, but it's a *felt-experience* question the owner's own cold playtest is specifically positioned to answer, and building any of it now would pre-empt that data. This retune touches values only.

**In scope, sim-verify each via propose-then-ratify (same discipline as every prior SCOPED UNLOCK):**
- Salary ratio: bring 62–69.5% (human/casual) back inside the 30–55% band.
- Research/Flight XP/Reputation abundance: reduce generation and/or raise costs enough that the ENTIRE tree (Research), all 4 XP trees (Flight XP), and the highest gate (Reputation) don't get trivially exceeded days before the content that spends them is even reached. Target: none of the three should be "solved" (all content affordable) before Aurora II, at minimum.
- `orbitalFlight` / Aurora II separation: the gate needs to bind meaningfully AFTER Aurora I's success, not just cost more Research (proven not to work — 700→1,000 R moved nothing). Candidate approaches: gate partly on Reputation or on Flight XP rather than Research alone (forcing genuine time/mission-count to pass, not just lab-output accumulation); or a flat minimum elapsed-time-since-Aurora-I floor. Propose against the sim, don't guess.
- Hiring-cost curve: re-open per §3d's fresh data — the 1.15→1.20 change didn't resolve the finding.
- Contract pay: raise enough that fulfilling contracts is a meaningful fraction of income post-Aurora-I, not 0.22%. Tier-2 in particular ("the program's most lucrative clients" per GDD §6) should actually read as lucrative relative to passive income at the point it unlocks.

**Explicitly OUT of scope for this pass:** any new mechanic, sink, node, or system. If the retune above still leaves the arc feeling short once fixed, THAT's the signal for the cold playtest to surface — don't pre-solve it with guessed content now.
...
**Resolved (Sprint 11.5), shipped in code (`researchTree.ts`):** (a) `probe1Engine` gained `buildingDep: testStand` + a 50-Materials secondary cost; `orbital1Engine` gained 200 Materials (transitively covered by probe1Engine's buildingDep). (b) Secondary costs on 5 nodes total: `probe1Engine`/`orbital1Engine`/`autoRefuel` (Materials), `testStand`/`flightProgram` (Funding) — everything else stayed pure-Research. (c) `orbitalFlight` raised 700→1,000R (the capstone node the abundance finding named). **Per v4.2's intro and §3e: this alone did not resolve the abundance problem** — Research is now 75× the entire tree's cost by day 12 regardless, which is why §3e's retune and §5c's redesign both exist. Don't re-read this as "already fixed."

## 5c. Research tree — major redesign & expansion (v4.3, owner direction — real content work, sequenced BEFORE the cold playtest per explicit owner sequencing, an exception to §3e's "no new content until playtest data" rule)
Real finding, from the owner's own testing plus genre research: the current ~15-node, 4-branch, fully-linear tree has "zero impact" — mostly gate-only nodes and small percentage bumps, nothing that changes how the game plays. This independently corroborates the dual-audit's finding that content runs dry mid-arc (§3e) — a KSP-community-cited reason "lack of purpose and direction" is a top complaint, and the standard antidote is a richer tech tree, which is what this section builds. This is a genuine exception to §3e's "numbers only, no new content" boundary — the owner explicitly chose to sequence this before further testing, having already gathered real play signal that the tree itself (not just its balance) is the problem.

**Redesign principles (binding; exact node list/costs go through the normal propose-then-ratify + sim sweep process, same as every other content addition):**
1. **Real expansion, not just rebalance:** roughly double-to-triple total node count (from ~15 toward ~30-40) across the 4 existing branches — this is what actually gives the "banked but unspent" Research (§3e: 75× tree cost by day 12) somewhere real to go, alongside that section's abundance-reduction work.
2. **At least one genuine mutually-exclusive fork per branch** (Total War: Three Kingdoms model, not KSP's "eventually unlock everything" model): picking node A permanently forecloses sibling node B for that save. This is a materially different kind of choice than sequencing — it defines playstyle identity and adds real replay value. First candidate already sketched in BACKLOG (Aggressive vs. Safety-margin fuel mixture, Propulsion branch) — needs 3-4 siblings across the other branches.
3. **At least 2 mechanic-changing nodes per branch, not 1** — directly closes the audit-found GDD §9 violation (Organization/Prestige currently have zero). A mechanic-changing node does something a modifier percentage can't: changes a process, unlocks a new interaction, changes what the player DOES, not just a number they watch.
4. **Cross-branch prerequisites for some late nodes** — at least 2-3 nodes should require a prerequisite from a DIFFERENT branch than their own, breaking the "4 fully independent parallel chains" structure without a full rewrite. E.g., a late Propulsion node requiring an Operations node first.
5. **A genuine repeatable end-node per branch** (buy multiple times, escalating cost, real but diminishing returns) — the direct fix for "310,000 banked Research with nothing to spend it on": an infinite sink absorbs abundance by design rather than needing generation nerfed to zero. This pairs with, doesn't replace, §3e's abundance-reduction work — both matter.
6. **Visual weight matches node importance (UI_SPEC §1d, new rule):** a fork or mechanic-changing node renders larger/more visually distinct in the lane layout (UI_SPEC screen 3) than a plain percentage node — the tree itself must not repeat the "uniform cards regardless of importance" problem UI_SPEC §1d identifies, just applied to nodes instead of buildings.

**Sequencing note:** this changes the previously-stated plan (ECONOMY §3e said no new content before the cold playtest) — the owner explicitly chose this order. Sprint 11.6's numbers-only retune (§3e) still applies to salary/hiring/contracts/Aurora-II-gating; THIS section is the one deliberate content exception, scoped narrowly to the research tree because it's where the owner's own testing (not just the sim) found a real design gap, not a balance one.
