...
**v4.1 changes (Sprint 11 balance pass — both open questions resolved with data, not assumption):**
- **Flight Data share (§8): metric redefined, no economy value changed.** The 20-35% target now applies only to preFlight (no target — lab-only, unchanged) and sonda; "satellite" era (Aurora I onward) is reporting-only, no target. Tried scoping the target to a "pre-Aurora-II" sub-window first — found via `sim/run.ts` instrumentation that window is EMPTY in every profile (Aurora II always launches the same simulated day as Aurora I; the bot re-queues its VAB immediately, no dead time). The 20-35% band predates Aurora II existing at all; once any satellite-class mission is flying, Flight Data dominating Research income (confirmed 81-85% across all 3 profiles, unchanged from Sprint 10) is the expected reward for reaching orbit, not a balance problem — there was never a real "not yet repeatable" window to hold a target against.
- **Hiring-cost curve (§3): confirmed a non-issue, closed without changing the curve.** `sim/run.ts` now reports final headcount + lifetime hiring/promotion spend per run: all 3 profiles converge to the identical 18/20 hired (staff-cap constrained, from Crew Quarters investment), having spent only ~3,700-4,000 total Funding on hiring across a full 30-day run — negligible against the millions in lifetime Funding those runs earn. The `1.15^hiredOfThatRole` curve never binds in practice: the staff cap and the 50%-of-passive-income salary budget (`resolveHiring()`) hit first, every time, regardless of profile. Steepening the curve would be a no-op under current v1 content depth. BACKLOG's "hiring-cost curve steeper" item closed as resolved, not deferred.

**v4.0 changes (Sprint 10 wrap-up):** satellite-era Flight Data share flagged at 81-85% vs the 20-35% target once Aurora II's repeatable, non-decaying reward is live — target predates Aurora II; framed for Sprint 11's balance pass as "redefine the metric vs. fix the economy," not assumed broken. See SPRINTS Sprint 11 for the full framing. *(Resolved above, v4.1.)*
...
- v3.9: §7 restructured — Aurora II confirmed to reuse Aurora I's mechanics wholesale; `orbitalFlight` tech gates the SECOND orbital attempt onward, not Aurora I's own launch; "Parallel integration" XP node clarified as auto-chaining with no dead time.
- v3.8: building-expansion milestone rule (§4c) — every 10 levels, +1 slot per role already employed, universal and automatic.
- v3.7: rate-display precision fixed (§12, deltas below 0.1 show 2 decimals); Basic engineering's cost raised to protect promotion-bootstrap pacing.
...
**v4.1: hiring-cost curve confirmed NOT the binding constraint** (see changelog above) — the staff cap (Crew Quarters) and the salary-budget check in hiring logic bind first in every profile; curve left unchanged.
...
Design targets (checked in `sim/run.ts`, human profile, reported per era): **Flight Data = 20–35% of total Research income during the sonda era** (pre-flight era is naturally lab-only, no target either); **pacing floor: the human profile must not reach Aurora I before simulated day 5.** The satellite era (Aurora I onward) is reporting-only as of v4.1 — no target, Flight Data is expected to dominate Research income once orbital missions are flying (see v4.1 changelog above for why the target doesn't carry forward). Failure never pays contract money or Reputation — gambling saves time, it never beats investing in Confidence on expected value.
