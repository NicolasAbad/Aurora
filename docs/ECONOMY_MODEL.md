...
**v3.6 changes (Sprint 8 economy unlock — SCOPED, per SPRINTS.md task 5 / BACKLOG.md "Parked for the Sprint 8 economy unlock"):** the parked Campus/Production internal-upgrade set is now priced and specified (§4) — Grants desk, Technical archive, Second research track, Bulk contracts, Recovery loop, QA station, Inventory system. Aluminum alloys (§5) gains a real effect, closing its own contingency-met BACKLOG item. Prices proposed against existing internal-upgrade cost bands (Campus/early-Production tier ≈ Classroom/Cafeteria's few-hundred-F range, not Launch complex's thousands); flagged for owner sign-off after implementation + a full sim-sweep re-run, same process as Sprint 7.5's Test Stand/Auto-refuel/basicLogistics SCOPED UNLOCKs. **No other economy value changed by this pass.**
...
**v3.6 additions (Sprint 8 economy unlock — priced against the Campus-tier band above, Classroom/Cafeteria, not the Launch-complex thousands):**
Finance: Grants desk 350 F (+1 Technician slot)
R&D Lab: Technical archive 500 F (+1 Scientist slot — priced above the flat Technician slot-adders: Scientists are the scarcer, costlier role and the Research bottleneck matters most) · Second research track 1,000 F (**a second research node can run in parallel with the first, resolved independently on its own timer** — the depth item of this set, priced as a real investment, not a throwaway; sim does not model it, same documented restraint as every other internal upgrade the bot doesn't buy)
Supply Depot: Bulk contracts 350 F (+1 Technician slot — same tier as Grants desk, same role)
Refinery: Recovery loop 550 F (−10% Materials consumed per Propellant)
Fabrication: QA station 600 F+100 M (−15% Materials consumed per Hardware; **stacks multiplicatively with Aluminum alloys' new §5 effect**, same stacking pattern as Instrumentation × Test Stand leveling)
Warehouse: Inventory system 700 F+150 M (+25% to the cap bonus each Warehouse level grants, from the level after purchase onward — not retroactive to already-owned levels, same "computed at the moment, never retroactive" precedent as Sprint 7.5's Auto-refuel/Test-Stand)
...
Materials: **Aluminum alloys** (25 R, 5 min — **v3.6: −10% Materials consumed per Hardware at Fabrication** (contingency-met per BACKLOG: the Materials branch was thin at 2 nodes; stacks multiplicatively with Fabrication's new QA station upgrade, §4). The Aluminum Hardware tier itself is still available from the start with NO tech required — this node's tier-gating and Titanium-prerequisite role is unchanged. Flavor: certifying aluminum stock for flight hardware) → Titanium (400 R, 3 h)
