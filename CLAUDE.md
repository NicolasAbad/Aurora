...
13. **Trigger conditions must be checked against CURRENT disclosure state, not just their own original condition.** A real bug found this way: T-02 fired on "50 Funding" alone, written before the Campus staged-reveal rules (UI_SPEC §2d) existed — it could fire before hiring was even unlocked. Any FTUE tooltip, Current Directive line, or milestone trigger written against an older/simpler rule must be re-verified whenever the disclosure sequence around it changes, not just left as originally written. When adding or changing a staged-reveal rule, grep for every tooltip/directive that could fire in that same window and confirm none of them assume a looser precondition than what's now gating the underlying feature.
