...
// ---------------------------------------------------------------------------
// Sprint 11.5: was this file's own hardcoded copy (predating data/roles.ts's own
// existence); now imports the real ROLES/PROMOTIONS so the two can't drift — same
// migration Sprint 4/6/7 already did for RESEARCH_TREE/SOUNDING_ROCKETS/certifications
// below. This divergence was harmless while ROLES was just cost/salary/unlockTech, but
// Sprint 11.5's promotion-only restructuring (Engineer/Scientist have no baseCost at
// all now, ECONOMY §3 v4.1) made a stale local copy actively dangerous (NaN hiring cost).
...
function resolveHiring(state: SimState): void {
  ...
  for (let guard = 0; guard < 1; guard++) {
    ...
    for (const role of Object.keys(ROLES) as RoleId[]) {
      ...
      // ECONOMY §3 v4.1 (Sprint 11.5): Engineer/Scientist have no direct-hire path at
      // all anymore — resolvePromotions is their only route (see its own header note).
      if (!def.hireable) continue;
...
// Promotion (ECONOMY §3 v4.1) is the ONLY route to Engineer/Scientist, full stop — there
// is no direct-hire path at all anymore (GDD §2 v2.11). Scientific method/Basic
// engineering can ONLY be researched via a Scientist-staffed R&D Lab, so bootstrapping
// still has to happen via promotion BEFORE either tech exists: Research is generated by
// promoting up to a Scientist first, same real bootstrap the game has always required
// (previously ALSO the only way around a hiring deadlock; now the only way, period).
// ECONOMY §5 v4.1 (Sprint 11.5): basicEngineering/scientificMethod are now promotion
// accelerators (-25% cost+duration each), not hiring-unlock techs — same conditional-
// effect style this file already uses for every other research bonus (autoRefuelMult,
// basicLogisticsMult, materialsPerHardware above), rather than a generic modifier system.
const PROMOTION_ACCELERATOR_MULT = 0.75;
...
function resolvePromotions(state: SimState): void {
  ...
  if (state.staffHired.engineer > 0 && canAfford(state, { funding: scientistCost })) {
    ...
    state.promotionTimer = {
      remainingMs: ENGINEER_TO_SCIENTIST.durationMs * scientistMult,
      onComplete: () => {
  ...
  state.day.fundingSpentOnHires += techHireCost + engineerPromoCost;
  state.lifetimeFundingSpentOnHires += techHireCost + engineerPromoCost;
  state.staffHired.technician += 1;
  state.promotionTimer = {
    remainingMs: TECH_TO_ENGINEER.durationMs * engineerMult,
    onComplete: () => {
      state.staffHired.technician -= 1;
      state.staffHired.engineer += 1;
